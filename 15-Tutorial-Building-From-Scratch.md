...
Next, configure `package.json` to tell React Native about your native module and, crucially, to configure CodeGen.
...
This is the **source of truth**. Create a file at `src/specs/NativeDeviceInfoModule.ts`.
...
On iOS, the implementation requires an `.mm` file. This is **Objective-C++**, a language that can understand both the C++ generated by CodeGen and the Objective-C of the native iOS frameworks (like `UIDevice`).
...
@interface DeviceInfoModule : RCTEventEmitter <NativeDeviceInfoModuleSpec>
...
Create a file at `src/specs/DeviceInfoViewNativeComponent.ts`.
...
For a Fabric component, we need a `Manager` and a `View`.
...
Similarly, Android needs a `Manager` and a custom `View`.
